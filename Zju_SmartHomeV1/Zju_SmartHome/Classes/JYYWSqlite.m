...
@interface JYYWSqlite()
{
    //声明一个sqlite3的数据库
    sqlite3 *db;
}
@end
@implementation JYYWSqlite
//返回数据库在文件夹中的全路径信息
-(NSString *)filePath
{
...
//创建表(rValue代表冷暖，gValue代表亮度)
-(void)createTable
{
    //模式名称，模式描述，模式图片
    NSString *sql=@"create TABLE if not EXISTS patternYWTable(name NSString,logoName NSString,bkgName NSString,rValue NSString,gValue NSString);";
    char *errorMesg=NULL;
...
{
    NSString *sql=[NSString stringWithFormat:@"INSERT INTO '%@'('%@','%@','%@','%@','%@') VALUES('%@','%@','%@','%@','%@')",tableName,field1,field2,field3,field4,field5,field1Value,field2Value,field3Value,field4Value,field5Value];
    char *err;
